    ...
    <section className="container mx-auto py-12 px-4">
      ...
      <div className="flex flex-wrap justify-evenly items-center gap-y-10 mt-12">
        {brands.map((brand, index) => (
          <div
            key={index}
            className="flex items-center px-5 justify-evenly w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/6 cursor-pointer"
            onClick={() => navigate(`/products?name=${encodeURIComponent(brand.name)}`)}
          >
            <img
              src={brand.logo}
              alt={`${brand.name} logo`}
              className="max-h-12 max-w-full object-contain hover:opacity-80 transition-opacity"
            />
          </div>
        ))}
      </div>
